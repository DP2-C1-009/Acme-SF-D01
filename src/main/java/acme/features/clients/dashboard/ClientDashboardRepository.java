...
public interface ClientDashboardRepository extends AbstractRepository {
	...
	@Query("select avg(c.budget.amount) from Contract c where c.client = :client and c.budget.currency= :ce and c.draftmode=false")
	Double findAverageBudgetOfContracts(Client client, String ce);

	@Query("select stddev(c.budget.amount) from Contract c where c.client = :client and c.budget.currency= :ce and c.draftmode=false")
	Double findDeviationBudgetOfContracts(Client client, String ce);

	@Query("select min(c.budget.amount) from Contract c where c.client = :client and c.budget.currency= :ce and c.draftmode=false")
	Double findMinimunBudgetOfContracts(Client client, String ce);

	@Query("select max(c.budget.amount) from Contract c where c.client = :client and c.budget.currency= :ce and c.draftmode=false")
	Double findMaximumBudgetOfContracts(Client client, String ce);
